...
- SQLite 连接与 schema 兼容逻辑位于 `src/kb/store.py`；FTS5、哈希向量、检索补召回和 `search_rows` 位于 `src/kb/indexing.py`。后续权威级别排序、中文 embedding 替换和检索质量测试优先改这些模块。
- 检索结果必须透出来源级别；`L1-L3` 是事实层，`L4` 是公众号写作样本/线索层。新增检索、引用表或素材包时，保留 `row_authority_label` 这类标识，不把 L4 默认写成史实定本。
